...
def save_image(file: pathlib, fold: int, images: list, model: str, patch: int,
...
def save_features_npz(fold: int, features: np.ndarray, model_name: str, n_patches: int,
                      path: LiteralString | pathlib.PurePath | str, ) -> None:
    path = create_path(path, 'features', model_name, 'npz')
    filename = 'fold-%d_patches-%d.npz' % (fold, n_patches)
    filename = os.path.join(path, filename)
    np.save(filename, features, allow_pickle=True)


def save_features_npy(fold: int, features: np.ndarray, model_name: str, n_patches: int,
                      path: LiteralString | pathlib.PurePath | str, ) -> None:
    path = create_path(path, 'features', model_name, 'npy')
    filename = 'fold-%d_patches-%d.npy' % (fold, n_patches)
    filename = os.path.join(path, filename)
    np.save(filename, features, allow_pickle=True)


def save_features_info(input: LiteralString | pathlib.PurePath | str, model_name: str,
                       path: LiteralString | pathlib.PurePath | str) -> None:
    for filename in ['info_dataset.csv', 'info_levels.csv', 'info_samples.csv']:
        if os.path.exists(os.path.join(input, filename)):
            ...
            dst = os.path.join(path, 'features', model_name, filename)
            shutil.copy(src, dst)
...
def save_features(features: np.ndarray,
                  ...
                  n_patches: int,
                  path: LiteralString | pathlib.PurePath | str) -> None:
    save_features_npz(fold, features, model_name, n_patches, path)
    save_features_npy(fold, features, model_name, n_patches, path)
    save_features_info(input, model_name, path)
...
def save_samples(filenames: list, model_name: str, path: LiteralString | pathlib.PurePath | str) -> None:
    columns = ['filename', 'fold']
    filename = os.path.join(path, 'features', model_name, 'info_samples2.csv')
...
def save_levels(levels: list, model_name: str, path: LiteralString | pathlib.PurePath | str) -> None:
    columns = ['levels', 'count', 'f']
    filename = os.path.join(path, 'features', model_name, 'info_levels2.csv')
...
def save_information(color: str,
    ...
    filename = os.path.join(path, 'features', model, 'info.csv')
    print('%s saved' % filename)
    df.to_csv(filename, sep=';', quoting=2, header=False, lineterminator='\n')

    save_samples(filenames, model, path)
    save_levels(levels, model, path)
